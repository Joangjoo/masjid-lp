...
import { galleryPhotos as fallbackPhotos } from '../../data/landingData';
import { SectionHeading } from '../common/SectionHeading';
import { apiService } from '../../services/api';

export const PhotoGalleryGrid: React.FC = () => {
  const [photos, setPhotos] = useState<any[]>(fallbackPhotos);
  ...
  const featuredPhoto = photos.find((p) => p.is_featured || p.featured) || photos[0];
  const regularPhotos = photos.filter((p) => p !== featuredPhoto);
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
          ...
          <div className="flex flex-col gap-6">
            {featuredPhoto && (
              ...
              >
                <img
                  src={featuredPhoto.image_url || featuredPhoto.imageUrl}
            ...
            {regularPhotos.slice(2, 3).map((photo, idx) => (
              <motion.div
                key={photo.id || idx}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
