...
interface NavbarProps {
  currentTab?: string;
  onSelectTab?: (tabHref: string) => void;
}
    ...
    >
      <div className="max-w-[1280px] mx-auto px-6 md:px-12 flex items-center justify-between">
        ...
        <a 
          href="#beranda" 
          onClick={() => {
            if (onSelectTab) onSelectTab('#beranda');
            setMobileMenuOpen(false);
          }}
        ...
        <nav className="hidden lg:flex items-center gap-7">
          {navLinks.map((link) => {
            const isActive = currentTab === link.href;
            return (
              <a
                key={link.label}
                href={link.href}
              ...
              {navLinks.map((link) => (
                <a
                  ...
                  className={`text-base font-medium py-2 border-b border-white/5 ${
                    currentTab === link.href ? 'text-[#C49B5E] font-semibold' : 'text-white/90 hover:text-[#C49B5E]'
                  }`}
                >
                  {link.label}
                </a>
